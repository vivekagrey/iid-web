...
def home_view(request):
    ...
    return render(request, 'app/home.html', {'calendar': calendar})
...
def contact_view(request):
    if request.method == "POST":
        ...
        obj = Query.objects.create(first_name=first_name, last_name=last_name, email=email, phone=phone, query=query)
        obj.save()
        if obj:
...
def course_detail(request, slug):
    ...
    if slug == 'dpc2':  # add slug in ''
        return render(request, 'app/dpc2.html', {'course': course})  # change 'filename'

    elif slug == 'domp': # add slug in ''
        return render(request, 'app/domp.html', {'course': course})  # change 'filename'
...
def gallery(request):
    ...
    paginator = Paginator(batches, 3)  # no. of batches per page
    try:
